...
/-- The candidate carrier set for `mem_generated_iff`: elements bounded in
absolute value by a non-negative linear combination of `|y|`'s for `y ∈ s`.
This set is shown to be an order ideal, and to coincide with `generated s`. -/
