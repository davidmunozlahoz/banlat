...
- The bundled structure `Band` extending `OrderIdeal`.
- The chain `Band → OrderIdeal → VectorSublattice`: every band is an order
  ideal and hence a vector sublattice. The underlying subtype therefore
  inherits the pointwise vector-lattice structure already established for
  vector sublattices, with no further work.
- The basic membership lemmas (closure under `⊔`, `⊓`, solidity, absolute
  value).
- A useful reformulation: an order ideal is a band as soon as its closure
  property holds for **positive directed** subsets; the closure for arbitrary
  subsets follows automatically.
