...
- The sup- and inf-closure of a pointed cone, which are themselves pointed cones in a
  vector lattice.
- The bundled structure `VectorSublattice` with its `SetLike` instance and the canonical
  coercion to a pointed cone.
- The lattice of vector sublattices of a given vector lattice `X`: top element `⊤ = X`,
  bottom element `⊥ = {0}`, and closure under arbitrary intersections. From these, the
  vector sublattice **generated** by a set `s ⊆ X` is the infimum of all vector sublattices
  containing `s`.
- Descriptions of the generated sublattice: for a submodule `M`, it coincides with
  `supClosure(infClosure M)`; for a pointed cone `C`, with the difference set of the
  sup-closure (or inf-closure) of `C`; for a pairwise-disjoint family of non-negative
  vectors, with its linear span.
- In the normed setting: the induced normed vector lattice structure on the subtype, the
  topological closure of a vector sublattice, and the separability of the closed vector
  sublattice generated by a countable subset.
-/
...
/-- Auxiliary: an element of `supClosure s` with sup over attached set is in the closure of
`supClosure D` when `s ⊆ closure D`. -/
