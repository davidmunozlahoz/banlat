...
/-! ### Normed and Banach lattice structure on the subtype of a band -/

section Normed

variable {X : Type*} [NormedAddCommGroup X] [Lattice X] [IsOrderedAddMonoid X]
  [NormedVectorLattice X] (B : Band X)

/-- The subtype of a band inherits solidity of the norm from the ambient space. -/
instance instHasSolidNormSubtype : HasSolidNorm ↥B.toSubmodule where
  solid {x y} h := by
    change ‖x.val‖ ≤ ‖y.val‖
    exact norm_le_norm_of_abs_le_abs (h : |x.val| ≤ |y.val|)

/-- The subtype of a band inherits the homogeneity of the norm from the ambient
space. -/
instance instNormSMulClassSubtype : NormSMulClass ℝ ↥B.toSubmodule where
  norm_smul r x := by
    change ‖r • x.val‖ = ‖r‖ * ‖x.val‖
    exact norm_smul r x.val

/-- The subtype of a band is itself a normed vector lattice. -/
instance instNormedVectorLatticeSubtype :
    NormedVectorLattice ↥B.toSubmodule where

/-- In a normed vector lattice, the disjoint complement of any set is norm
closed: it is an intersection of closed sets `{x | |x| ⊓ |a| = 0}`, each of
which is the zero-set of the continuous map `x ↦ |x| ⊓ |a|`. -/
theorem isClosed_disjointComplement (A : Set X) : IsClosed (Aᵈ) := by
  have heq : Aᵈ = ⋂ a ∈ A, {x : X | |x| ⊓ |a| = 0} := by
    ext x
    simp only [disjointComplement, Set.mem_setOf_eq, Set.mem_iInter, IsVLDisjoint]
  rw [heq]
...
/-- **Every band in a normed vector lattice is norm closed.** A normed vector
lattice is Archimedean, so any band equals its double disjoint complement
`(Bᵈ)ᵈ`, and disjoint complements are closed by
`isClosed_disjointComplement`. -/
theorem isClosed_coe : IsClosed ((B : Set X)) := by
  rw [← eq_disjointComplement_disjointComplement B]
  exact isClosed_disjointComplement _

/-- The underlying submodule of a band is norm closed. -/
theorem isClosed_toSubmodule : IsClosed ((B.toSubmodule : Set X)) := B.isClosed_coe

/-- A band in a Banach lattice, equipped with the inherited subtype structure,
is itself a Banach lattice. The underlying submodule is norm closed
(`isClosed_coe`), hence complete. -/
noncomputable instance instBanachLatticeSubtype [BanachLattice X] :
    BanachLattice ↥B.toSubmodule :=
  haveI : IsClosed ((B.toSubmodule : Submodule ℝ X) : Set X) := B.isClosed_toSubmodule
  haveI : CompleteSpace ↥B.toSubmodule := inferInstance
  {}
...
instance (priority := 100) HasPrincipalProjectionProperty.of_hasProjectionProperty
  ...
  exists_projectionBand a :=
    HasProjectionProperty.exists_projectionBand (Band.principalBand a)
...
instance (priority := 100) HasProjectionProperty.of_isOrderComplete
    [IsOrderComplete X] : HasProjectionProperty X :=
  sorry
...
instance (priority := 100)
    ...
    [IsSigmaOrderComplete X] : HasPrincipalProjectionProperty X :=
  sorry
...
theorem isMaximalDisjoint_iff_disjointComplement_eq_zero {Λ : Set X}
    ...
    IsMaximalDisjoint Λ ↔ Λᵈ = ({0} : Set X) := by
  constructor
  · -- Forward: maximal ⇒ Λᵈ = {0}
    intro hmax
    apply Set.Subset.antisymm
    · intro x hx
      rw [Set.mem_singleton_iff]
      -- Suppose x ≠ 0, derive contradiction
      by_contra hx_ne
      have habs_pos : 0 < |x| := by
        rcases lt_or_eq_of_le (abs_nonneg x) with h | h
        · exact h
        · exact absurd ((abs_eq_zero_iff_zero x).mp h.symm) hx_ne
      -- Λ ∪ {|x|} is still a valid disjoint family of positives
      set Λ' := Λ ∪ {|x|}
      have hpos' : ∀ y ∈ Λ', 0 < y := by
        rintro y (hy | hy)
        · exact hpos y hy
        · rw [Set.mem_singleton_iff] at hy; rw [hy]; exact habs_pos
      have hdis' : IsDisjointSet Λ' := by
        rintro a (ha | ha) b (hb | hb) hab
        · exact hdis ha hb hab
        · rw [Set.mem_singleton_iff] at hb; rw [hb]
          have := hx a ha
          rw [show (|x| : X) = |x| from rfl]
          unfold IsVLDisjoint
          rw [abs_abs]
          rw [isVLDisjoint_comm] at this
          exact this
        · rw [Set.mem_singleton_iff] at ha; rw [ha]
          have := hx b hb
          unfold IsVLDisjoint
          rw [abs_abs]
          exact this
        · rw [Set.mem_singleton_iff] at ha hb; rw [ha, hb] at hab; exact (hab rfl).elim
      have hsub : Λ ⊆ Λ' := Set.subset_union_left
      have hΛ'_le : Λ' ⊆ Λ := hmax.le_of_ge ⟨hpos', hdis'⟩ hsub
      have habs_in : |x| ∈ Λ := hΛ'_le (Or.inr rfl)
      -- x ⊥ |x| since |x| ∈ Λ and x ∈ Λᵈ
      have := hx (|x|) habs_in
      unfold IsVLDisjoint at this
      rw [abs_abs, inf_idem] at this
      exact absurd ((abs_eq_zero_iff_zero x).mp this) hx_ne
    · intro x hx
      rw [Set.mem_singleton_iff] at hx
      rw [hx]
      intro a _
      exact isVLDisjoint_zero_left a
  · -- Backward: Λᵈ = {0} ⇒ maximal
    intro hΛd
    refine ⟨⟨hpos, hdis⟩, ?_⟩
    rintro S ⟨hpos_S, hdis_S⟩ hsub x hx
    by_contra hx_notin
    have hx_disj : ∀ y ∈ Λ, IsVLDisjoint x y := fun y hy => by
      have hxy_ne : x ≠ y := fun heq => hx_notin (heq ▸ hy)
      exact hdis_S hx (hsub hy) hxy_ne
    have : x ∈ Λᵈ := hx_disj
    rw [hΛd, Set.mem_singleton_iff] at this
    exact absurd this (ne_of_gt (hpos_S x hx))
...
theorem exists_isMaximalDisjoint (X : Type*) [AddCommGroup X] [Lattice X]
    [IsOrderedAddMonoid X] [VectorLattice X] :
    ∃ Λ : Set X, IsMaximalDisjoint Λ := by
  set 𝒮 : Set (Set X) :=
    {S : Set X | (∀ x ∈ S, 0 < x) ∧ IsDisjointSet S}
  have hchain : ∀ c ⊆ 𝒮, IsChain (· ⊆ ·) c →
      ∃ ub ∈ 𝒮, ∀ s ∈ c, s ⊆ ub := by
    intro c hc_sub hchain
    refine ⟨⋃₀ c, ?_, fun s hs x hx => ⟨s, hs, hx⟩⟩
    refine ⟨?_, ?_⟩
    · rintro x ⟨S, hSc, hxS⟩
      exact (hc_sub hSc).1 x hxS
    · rintro a ⟨A, hAc, haA⟩ b ⟨B, hBc, hbB⟩ hab
      rcases hchain.total hAc hBc with hAB | hBA
      · exact (hc_sub hBc).2 (hAB haA) hbB hab
      · exact (hc_sub hAc).2 haA (hBA hbB) hab
  obtain ⟨Λ, hΛ⟩ := zorn_subset 𝒮 hchain
  exact ⟨Λ, hΛ⟩
...
theorem isWeakOrderUnit_principalBand_self [IsVLArchimedean X] {a : X}
    (_ha : 0 ≤ a) :
    ∀ x ∈ Band.principalBand a, IsVLDisjoint x a → x = 0 := by
  intro x hx hdis
  -- x ∈ principalBand a = bandGenerated {a} = {a}ᵈᵈ (Archimedean)
  have hx_dd : x ∈ (({a} : Set X)ᵈ)ᵈ := by
    rw [Band.disjointComplement_disjointComplement_eq_bandGenerated]
    exact hx
  -- x ∈ {a}ᵈ from disjointness
  have hx_d : x ∈ ({a} : Set X)ᵈ := by
    intro b hb; rw [Set.mem_singleton_iff] at hb; rw [hb]; exact hdis
  -- intersection ⊆ {0}
  exact Set.mem_singleton_iff.mp
    (disjointComplement_inter_eq_zero (({a} : Set X)ᵈ) ⟨hx_d, hx_dd⟩)
...
theorem isLUB_principalBandProjection_of_isMaximalDisjoint
    ...
    {x : X} (hx : 0 ≤ x) :
    IsLUB (Set.range fun a : Λ => principalBandProjection (a : X) x) x := by
  refine ⟨?_, ?_⟩
  · -- Upper bound: each P_a x ≤ x
    rintro _ ⟨a, rfl⟩
    exact (principalProjectionBand (a : X)).bandProjection_le hx
  · -- Least: for any upper bound u, x ≤ u
    intro u hu
    -- Extract maximality data
    have hprop := hΛ.prop
    have hpos := hprop.1
    have hdis := hprop.2
    have hΛd : Λᵈ = ({0} : Set X) :=
      (isMaximalDisjoint_iff_disjointComplement_eq_zero hpos hdis).mp hΛ
    -- w := x - x ⊓ u ≥ 0 and we will show w ∈ Λᵈ.
    set w := x - x ⊓ u with hw_def
    have hw_nn : 0 ≤ w := by rw [hw_def]; exact sub_nonneg.mpr inf_le_left
    have hw_in : w ∈ Λᵈ := by
      intro a haΛ
      have hPaxu : Band.principalBandProjection a x ≤ u :=
        hu ⟨⟨a, haΛ⟩, rfl⟩
      have hPax_le_x : Band.principalBandProjection a x ≤ x :=
        (Band.principalProjectionBand a).bandProjection_le hx
      have hPax_le : Band.principalBandProjection a x ≤ x ⊓ u :=
        le_inf hPax_le_x hPaxu
      have hw_le : w ≤ x - Band.principalBandProjection a x := by
        rw [hw_def]; exact sub_le_sub_left hPax_le x
      have hres : x - Band.principalBandProjection a x ∈
          disjointComplement ((Band.principalProjectionBand a : ProjectionBand X) : Set X) :=
        (Band.principalProjectionBand a).id_sub_bandProjection_mem x
      rw [Band.principalProjectionBand_coe] at hres
      have ha_mem : a ∈ (Band.principalBand a : Set X) :=
        subset_bandGenerated _ (Set.mem_singleton a)
      have h1 : IsVLDisjoint (x - Band.principalBandProjection a x) a := hres a ha_mem
      have hxPax_nn : 0 ≤ x - Band.principalBandProjection a x :=
        sub_nonneg.mpr hPax_le_x
      unfold IsVLDisjoint at h1 ⊢
      rw [abs_of_nonneg hxPax_nn] at h1
      rw [abs_of_nonneg hw_nn]
      apply le_antisymm _ (le_inf hw_nn (abs_nonneg a))
      calc w ⊓ |a|
          ≤ (x - Band.principalBandProjection a x) ⊓ |a| :=
            inf_le_inf_right _ hw_le
        _ = 0 := h1
    rw [hΛd, Set.mem_singleton_iff] at hw_in
    have hxu : x = x ⊓ u := sub_eq_zero.mp hw_in
    rw [hxu]; exact inf_le_right
