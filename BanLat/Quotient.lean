...
private theorem archimedean_of_uniformlyClosed
    ...
    IsVLArchimedean (X ⧸ J.toSubmodule) where
  eq_zero_of_nonneg_of_forall_nsmul_le {φ ψ} hφ hsmul := by
...
noncomputable def quotientNormedVectorLattice
  ...
  exact {
    instQuotientVectorLattice J with
    solid := fun {a b} hab => by
      ...
      linarith [hγ_eq ▸ this, norm_le_norm_of_abs_le_abs hγ_abs]
...
noncomputable def quotientBanachLattice
  ...
  haveI : IsClosed (J.toSubmodule : Set X) := hJ
  { quotientNormedVectorLattice J hJ with
    complete := (inferInstance : CompleteSpace (X ⧸ J.toSubmodule)).complete }
