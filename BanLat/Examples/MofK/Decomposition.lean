...
This file contains the atomic and continuous part facts for `M(K)`, together
with the expensive countable-Dirac decomposition results.
-/
...
omit [TopologicalSpace K] [T2Space K] [CompactSpace K] [BorelSpace K] in
private theorem signedMeasure_apply_le_apply_of_subset {s : SignedMeasure K}
    (hs : 0 ≤ s) {A B : Set K}
...
private theorem signedMeasure_inf_signedDirac_eq_zero_iff
    {s : SignedMeasure K} (hs : 0 ≤ s) (x : K) :
    s ⊓ signedDirac x = 0 ↔ s ({x} : Set K) = 0 := by
  ...
  · set c : ℝ := min (s ({x} : Set K)) 1 with hc_def
    ...
    have hcd_le_s : c • signedDirac x ≤ s := by
      intro B hB
      rw [VectorMeasure.smul_apply, smul_eq_mul]
      by_cases hxB : x ∈ B
      · rw [signedDirac_apply_of_mem hB hxB, mul_one]
        calc
          c ≤ s ({x} : Set K) := min_le_left _ _
          _ ≤ s B := signedMeasure_apply_le_apply_of_subset hs hxset hB
              (Set.singleton_subset_iff.mpr hxB)
      · rw [signedDirac_apply_of_notMem hB hxB, mul_zero]
        have := hs _ hB
        rwa [VectorMeasure.zero_apply] at this
    have hcd_le_d : c • signedDirac x ≤ signedDirac x := by
      intro B hB
      rw [VectorMeasure.smul_apply, smul_eq_mul]
      by_cases hxB : x ∈ B
      · rw [signedDirac_apply_of_mem hB hxB, mul_one]
      ...
      · rw [signedDirac_apply_of_notMem hB hxB, mul_zero]
    have hcd_le_inf : c • signedDirac x ≤ s ⊓ signedDirac x :=
      le_inf hcd_le_s hcd_le_d
    rw [h] at hcd_le_inf
    have h_at_x := hcd_le_inf {x} hxset
    rw [VectorMeasure.smul_apply, smul_eq_mul,
      signedDirac_apply_of_mem hxset (Set.mem_singleton x), mul_one,
  ...
  · refine VectorMeasure.ext fun B hB => ?_
    ...
    have hinf_nn : (0 : SignedMeasure K) ≤ s ⊓ signedDirac x :=
      le_inf hs (zero_le_signedDirac x)
    have hinf_le_d : s ⊓ signedDirac x ≤ signedDirac x := inf_le_right
    have hinf_le_s : s ⊓ signedDirac x ≤ s := inf_le_left
    ...
    have h_split : (s ⊓ signedDirac x) (B ∩ {x}) +
        (s ⊓ signedDirac x) (B \ {x}) = (s ⊓ signedDirac x) B := by
      rw [← VectorMeasure.of_union h_disj hBint hBdiff, h_cup]
    have h_diff_zero : (s ⊓ signedDirac x) (B \ {x}) = 0 := by
      have h_ub := hinf_le_d _ hBdiff
      have hxnotMem : x ∉ B \ {x} := by simp
      rw [signedDirac_apply_of_notMem hBdiff hxnotMem] at h_ub
      have h_lb := hinf_nn _ hBdiff
      rw [VectorMeasure.zero_apply] at h_lb
      linarith
    have h_int_zero : (s ⊓ signedDirac x) (B ∩ {x}) = 0 := by
      by_cases hxB : x ∈ B
      · have h_eq : B ∩ {x} = {x} := by
          ext y
...
private theorem isVLDisjoint_dirac_iff_singleton_zero (μ : MofK K) (x : K) :
  ...
  have habs_dx : |signedDirac x| = signedDirac x := abs_of_nonneg (zero_le_signedDirac x)
  refine Iff.trans ?_ (signedMeasure_inf_signedDirac_eq_zero_iff (abs_nonneg _) x)
  constructor
  · intro h
    have hh : ((|μ| ⊓ |dirac x| : MofK K) : SignedMeasure K) = 0 := congrArg Subtype.val h
    change (|(μ : SignedMeasure K)| ⊓ |signedDirac x| : SignedMeasure K) = 0 at hh
    rwa [habs_dx] at hh
  · intro h
    apply Subtype.ext
    change (|(μ : SignedMeasure K)| ⊓ |signedDirac x| : SignedMeasure K) = 0
    rwa [habs_dx]
...
private theorem dirac_apply_of_notMem {x : K} {A : Set K} (hA : MeasurableSet A) (hx : x ∉ A) :
...
private theorem posPart_le_abs (μ : MofK K) : μ⁺ ≤ |μ| := by
  rw [← posPart_add_negPart μ]
  exact le_add_of_nonneg_right (negPart_nonneg μ)

private theorem negPart_le_abs (μ : MofK K) : μ⁻ ≤ |μ| := by
  rw [← posPart_add_negPart μ]
  exact le_add_of_nonneg_left (posPart_nonneg μ)
...
theorem exists_sum_dirac_add_noAtoms (μ : MofK K) :
  ...
  letI : ConditionallyCompleteLattice (MofK K) :=
