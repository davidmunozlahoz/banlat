...
private abbrev bandGenerated {X : Type*} [AddCommGroup X] [Lattice X]
    [IsOrderedAddMonoid X] [VectorLattice X] (s : Set X) : Band X :=
  Band.generated s

private theorem bandGenerated_le {X : Type*} [AddCommGroup X] [Lattice X]
    [IsOrderedAddMonoid X] [VectorLattice X] {s : Set X} {B : Band X} (h : s ⊆ B) :
    Band.bandGenerated s ≤ B :=
  Band.generated_le h

private theorem disjointComplement_disjointComplement_eq_bandGenerated
    {X : Type*} [AddCommGroup X] [Lattice X] [IsOrderedAddMonoid X]
    [VectorLattice X] [IsVLArchimedean X] (A : Set X) :
    ((Aᵈ)ᵈ : Set X) = (Band.bandGenerated A : Set X) :=
  Band.disjointComplement_disjointComplement_eq_generated A

private abbrev disjointComplementBand {X : Type*} [AddCommGroup X] [Lattice X]
    [IsOrderedAddMonoid X] [VectorLattice X] (A : Set X) : Band X :=
  Band.disjointComplement A

private theorem mem_disjointComplement_iff {X : Type*} [AddCommGroup X] [Lattice X]
    [IsOrderedAddMonoid X] [VectorLattice X] {A : Set X} {x : X} :
    x ∈ Aᵈ ↔ ∀ a ∈ A, IsVLDisjoint x a :=
  _root_.mem_disjointComplement_iff
