...
omit [MeasurableSingletonClass α] in
/-- For a vector-lattice atom of `SignedMeasure α`, every measurable set takes value either
...
/-- **Atoms of `SignedMeasure α` are exactly the positive multiples of Dirac deltas.**
A finite signed measure is a vector-lattice atom of `SignedMeasure α` iff it is a
strictly positive scalar multiple of some Dirac delta `δ_x`. This identifies
the atoms of the Banach lattice `SignedMeasure α` with the parametrised family
`{c • δ_x : 0 < c, x ∈ K}`.
