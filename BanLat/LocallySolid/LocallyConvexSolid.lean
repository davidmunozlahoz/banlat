...
private def solidCore (s : Set E) : Set E := {x | ∀ ⦃y : E⦄, |y| ≤ |x| → y ∈ s}

private theorem solidCore_subset (s : Set E) : solidCore s ⊆ s := fun _ hx => hx le_rfl

private theorem isSolid_solidCore (s : Set E) :
    LatticeOrderedAddCommGroup.IsSolid (solidCore s) := by
  intro x hx y hy z hz
  exact hx (hz.trans hy)

variable [IsOrderedAddMonoid E] [VectorLattice E]

private theorem convex_solidCore {s : Set E} (hs : Convex ℝ s) :
    Convex ℝ (solidCore s) := by
...
theorem hasBasis_convex_solid :
  ...
  refine ⟨solidCore C, ?_, ⟨convex_solidCore hC.2, isSolid_solidCore C⟩, ?_⟩
  · exact Filter.mem_of_superset hV.1 fun x hx y hy => hVC (hV.2 hx hy)
  · exact fun x hx => hSU (hCS (solidCore_subset C hx))
