...
* `mem_closure_principal_iff_tendsto_inf`: in a Banach lattice, a non-negative
  element `x` belongs to the closure of the principal ideal of `e ≥ 0` iff
  `x ⊓ (n : ℝ) • e → x` as `n → ∞`.
* `quasiInteriorPoint_iff_tendsto_inf`: `e` is quasi-interior iff
  `x ⊓ (n : ℝ) • e → x` for every `x ≥ 0`.
* `exists_quasiInteriorPoint_of_separable`: every separable Banach lattice
  admits a quasi-interior point.
* `quasiInteriorPoint_iff_weakOrderUnit`: in an order continuous normed vector
  lattice, an element is quasi-interior iff it is a weak order unit.
-/
